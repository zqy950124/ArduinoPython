...
from DHTSensor_monitor_thread        import ComMonitorThread
...
class PlottingDataMonitor(QMainWindow):
    ...
    def create_plot(self):
        ...
        plot.setAxisScale(Qwt.QwtPlot.yLeft, LeftYMIN, LeftYMAX, (LeftYMAX - LeftYMIN) / 5)

        plot.setAxisTitle(Qwt.QwtPlot.yRight, 'Humidity')
        plot.setAxisScale(Qwt.QwtPlot.yRight, RightYMIN, RightYMAX, (RightYMAX - RightYMIN) / 5)
        plot.replot()
    ...
    def create_tableview(self):
        ...
        Return:    return a table containing the list of the datas
        """
        table = QTableWidget()
        table.setRowCount(5)
        table.setColumnCount(4)
        table.setHorizontalHeaderLabels(['时间', '温度', '湿度', '体感温度'])
        return table
    ...
    def create_main_frame(self):
        ...
        self.gCheckBox = [self.create_checkbox("Temperature(t)", Qt.green, self.activate_curve, 0),
                          self.create_checkbox("Humidity(h)", Qt.red, self.activate_curve, 1),
                          self.create_checkbox("Heat Index(a)", Qt.blue, self.activate_curve, 2)
                         ]
       ...
       # (QWidget * widget, int fromRow, int fromColumn, int rowSpan, int columnSpan, Qt::Alignment alignment = 0)
        plot_layout.addWidget(self.plot, 0, 0, 5, 7)
        plot_layout.addWidget(self.table, 5, 0, 3, 7)
    ...
    def update_monitor(self):
        ...
        if self.livefeed.has_new_data:
           ...
           #     self.table.setItem(0, i, tableItem)

            cnt = str(data['timestamp'])
            tableItem = QTableWidgetItem(cnt)
            self.table.setItem(0, 0, tableItem)
            cnt = str(data['t'])
            tableItem = QTableWidgetItem(cnt)
            self.table.setItem(0, 1, tableItem)
            cnt = str(data['h'])
            tableItem = QTableWidgetItem(cnt)
            self.table.setItem(0, 2, tableItem)
            cnt = str(data['d'])
            tableItem = QTableWidgetItem(cnt)
            self.table.setItem(0, 3, tableItem)

            # save data -如果保存的数量大，保存时间长，需要改为线程
            self.csvdata.append([data['timestamp'], data['t'], data['h'], data['d']])
